...
struct Options {
    ...
    // Colorizer quality
    var colorizerQuality: STColorizerQuality = STColorizerQuality.ultraHighQuality
    
    // Take a new keyframe if the rotation difference is higher than 10 degrees.
    var maxKeyFrameRotation: CGFloat = CGFloat(10 * (Double.pi / 180)) // 10 degrees
    
    // Take a new keyframe if the translation difference is higher than 10 cm.
    var maxKeyFrameTranslation: CGFloat = 0.1 // 10cm
    ...
    var colorizerTargetNumFaces: Int = 65535
...
struct AppStatus {
    ...
    let needColorCameraAccessMessage = "This app needs color camera access"
    let needCalibratedColorCameraMessage = "This app needs color camera calibration"
    
    let finalizingMeshMessage = "Rendering mesh"
    let sensorIsWakingUpMessage = "Structure Sensor is starting"
    
    // Whether there is currently a message to show.
    var needsDisplayOfStatusMessage = true
...
struct DisplayData {
    ...
    var meshRenderingAlpha: Float = 1
...
class ScanViewController: UIViewController, STBackgroundTaskDelegate, ColorizeDelegate, ScanViewDelegate, UIGestureRecognizerDelegate, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    // FootSide information for display
    var footSide : FootSide? = nil
    ...
    func meshViewDidRequestColorizing(_ mesh: STMesh, previewCompletionHandler: @escaping () -> Void, enhancedCompletionHandler: @escaping () -> Void) -> Bool {
        ...
        do
        {
            _naiveColorizeTask = try STColorizer.newColorizeTask(with: mesh,
                                                                 ...
                                                                 options: [kSTColorizerTypeKey : STColorizerType.textureMapForObject.rawValue,
    ...
    func backgroundTask(_ sender: STBackgroundTask!, didUpdateProgress progress: Double) {
        ...
        let processingStringFormat = "Processing..."
    ...
    func respondToMemoryWarning() {
        ...
        case .viewing:
            ...
            if _enhancedColorizeTask != nil && !_slamState.showingMemoryWarning {
                ...
                let alertCtrl = UIAlertController(
                    ...
                    message: "Colorizing process was cancelled",
        ...
        case .scanning:
            ...
            if !_slamState.showingMemoryWarning {
                ...
                let alertCtrl = UIAlertController(
                    title: "Warning: Low memory",
                    message: "Scanning will be stopped",
                    preferredStyle: .alert)
    ...
    internal func showDistanceToTargetAndHeelIndicator(_ distance: Float) {
        if !distance.isNaN {
            let distanceFormatString = "Scanning \(self.footSide!.rawValue)"
            distanceLabel.isHidden = false
            distanceLabel.layer.cornerRadius = 8
            distanceLabel.layer.masksToBounds = true
            distanceLabel.text = distanceFormatString
