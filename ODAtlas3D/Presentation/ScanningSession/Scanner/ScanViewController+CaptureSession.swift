...
extension ScanViewController : STCaptureSessionDelegate {
    ...
    func videoDeviceSupportsHighResColor() -> Bool {
        ...
        // If your Structure Sensor is on firmware 2.0+, it supports depth capture at FPS of 24.
    ...
    func setupCaptureSession() {
        ...
        var resolution = _dynamicOptions.highResColoring ? STCaptureSessionColorResolution.resolution2592x1936 : STCaptureSessionColorResolution.resolution2048x1536

        if (!self.videoDeviceSupportsHighResColor())
        {
            os_log(.info, log: OSLog.sensor, "Device does not support high resolution color mode!");
            resolution = STCaptureSessionColorResolution.resolution2048x1536;
        }
